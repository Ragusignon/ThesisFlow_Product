...
export interface DocumentationPage {
  ...
  slug: string;
  content: string;
  category: 'Getting Started' | 'User Guide' | 'Admin Guide' | 'API Reference' | 'Troubleshooting' | 'FAQ';
  excerpt: string;
  lastUpdated: string;
  author: string;
  published: boolean;
...
const STORAGE_KEY = 'thesisflow_documentation';

// Default documentation pages
const defaultDocPages: DocumentationPage[] = [
  {
    id: 1,
...
export const getDocumentationPages = (): DocumentationPage[] => {
  try {
    ...
    localStorage.setItem(STORAGE_KEY, JSON.stringify(defaultDocPages));
...
// Get published documentation pages
export const getPublishedDocumentation = (): DocumentationPage[] => {
  return getDocumentationPages().filter(page => page.published);
};

// Get documentation by category
export const getDocumentationByCategory = (category: DocumentationPage['category']): DocumentationPage[] => {
...
export const getDocumentationBySlug = (slug: string): DocumentationPage | undefined => {
  return getPublishedDocumentation().find(page => page.slug === slug);
};

// Save documentation page
export const saveDocumentationPage = (page: DocumentationPage): void => {
  try {
    const pages = getDocumentationPages();
    const index = pages.findIndex(p => p.id === page.id);
    
    if (index !== -1) {
      pages[index] = { ...page, lastUpdated: new Date().toISOString().split('T')[0] };
    } else {
      pages.push({ ...page, lastUpdated: new Date().toISOString().split('T')[0] });
    }
    
    localStorage.setItem(STORAGE_KEY, JSON.stringify(pages));
  } catch (error) {
    console.error('Error saving documentation:', error);
    throw new Error('Failed to save documentation page');
  }
};

// Create new documentation page
export const createDocumentationPage = (): DocumentationPage => {
  const pages = getDocumentationPages();
  ...
  return {
    ...
    content: '<h2>New Documentation Page</h2><p>Start writing your documentation here...</p>',
...
export const deleteDocumentationPage = (id: number): void => {
  try {
    const pages = getDocumentationPages();
    ...
    throw new Error('Failed to delete documentation page');
  }
};

// Get all categories
export const getDocumentationCategories = (): DocumentationPage['category'][] => {
  return ['Getting Started', 'User Guide', 'Admin Guide', 'API Reference', 'Troubleshooting', 'FAQ'];
};
